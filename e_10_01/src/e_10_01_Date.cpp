...
//日付の不正値を補正させる関数の定義。
void Date::day_balance(Date& ymd)
{
	for( ;ymd.d > dmax[ymd.m % 12] || ymd.d < 1 ; ) {
		//日数がその月の範囲を超えるとき。
		if(ymd.d > dmax[ymd.m % 12]) {
			ymd.d -= dmax[ymd.m % 12];	//日数を減らす。
			ymd.m++;			//月を一つ加算し、
		} else if (ymd.d < 1) {
			ymd.d += dmax[ymd.m % 12];	//日数を増やす。
			ymd.m--;			//月を一つ減らす。
		}
	}

	for( ; ymd.m > 12 || ymd.m < 1; ) {
		//次に月の処理に移る。
		if(ymd.m > 12) {
			ymd.y++;		//年数を一つ加算し、、
			ymd.m -= 12;	//月から12を減算する。
