...
int main()
{
	...
	while (true) {
		...
		switch (selected) {
		//1の時は情報を表示。
		case 1: display(day[0]);		break;	// 日付に関する情報を表示
		case 2: change(day[0]);		break;	// 日付を変更
		case 3: inc_dec(day[0]);		break;	// 増分演算子・減分演算子
		case 4: before_after(day[0]);	break;	// 前後の日付を求める
		case 5: compare(day[0]);		break;	// 他の日付との比較
		}
	}
